...
import gleam/string
import gleam/uri
import http/api.{type MrCommit, type RepoCommits}
import http/lustre_http
import lustre/attribute as attr
import lustre/effect.{type Effect, batch, none}
import lustre/element.{type Element, text}
import lustre/element/html.{a, button, div, h2, li, ol, option, p, select, span}
import lustre/event
import modem
import routes
...
fn commit_timeline_item(
  ...
  let line_class = case is_last {
    True -> "hidden"
    False ->
      "absolute left-[1.125rem] top-9 bottom-0 w-px -translate-x-1/2 bg-slate-200"
  }
      ...
      div([attr.class(components.card <> " !p-0 overflow-hidden")], [
        div(
          ...
          [
            ...
            p([attr.class("text-xs text-gh-muted")], [
              text("Commits on " <> model.ref <> ", newest first"),
            ]),
            limited_note,
          ],
        ),
        ol(
          ...
          list.index_map(commits, fn(c, index) {
