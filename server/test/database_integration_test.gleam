...
pub fn create_org_integration_test() {
  db_test_support.with_db(fn(db) {
    ...
    let assert Ok(org) = database.create_org(db, "acme", "Acme Corp", "user_1")
...
pub fn ssh_keys_integration_test() {
  db_test_support.with_db(fn(db) {
    ...
    let assert True = string.contains(line, "GLEAMHUB_KEY_BLOB=" <> blob)
...
pub fn protected_branches_integration_test() {
  db_test_support.with_db(fn(db) {
    ...
    let assert Ok(branches) =
      database.replace_protected_branches(db, "acme", "demo", [
        "main",
        "release",
      ])
...
pub fn merge_request_lifecycle_integration_test() {
  db_test_support.with_db(fn(db) {
    ...
    let assert Ok(merged) =
      database.merge_merge_request(db, "acme", "demo", mr.number, sha, "user_1")
