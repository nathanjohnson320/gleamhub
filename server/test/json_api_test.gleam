import database.{
  KeyRow, MergeRequestCommentRow, MergeRequestRow, OrgRow, PipelineRunRow,
  RepoRow, UserRow, UserStatsRow,
}
import git/exec.{
  Blob, BlobContent, CommitEntry, DiffFile, MergeCheck, RepoTemplate, Submodule,
  Symlink, Tree, TreeEntry,
}
import gleam/json
import gleam/option
import gleam/string
...
pub fn access_json_test() {
  let s = json_api.access_json(True, False, "user_1") |> json_string
  let assert True = string.contains(s, "\"read\":true")
  let assert True = string.contains(s, "\"write\":false")
  let assert True = string.contains(s, "\"user_id\":\"user_1\"")
}
...
pub fn org_json_test() {
  let org =
    OrgRow(id: "org-1", slug: "acme", name: "Acme", role: option.Some("owner"))
...
pub fn repo_json_test() {
  ...
  let assert True =
    string.contains(s, "\"clone_url\":\"git@host:acme/demo.git\"")
...
pub fn me_json_test() {
  ...
  let stats =
    UserStatsRow(
      ...
      merged_merge_requests: 5,
      open_issues_authored: 1,
      open_issues_assigned: 3,
      reviews_given: 4,
    )
  let s = json_api.me_json(user, orgs, stats, 7) |> json_string
  let assert True = string.contains(s, "\"display_name\":\"Ada\"")
  let assert True = string.contains(s, "\"organizations\"")
  let assert True = string.contains(s, "\"unread_notifications\":7")
}
...
pub fn merge_request_json_test() {
  ...
  let s = json_api.merge_request_json(mr, "Ada") |> json_string
  let assert True = string.contains(s, "\"number\":1")
  let assert True = string.contains(s, "\"source_branch\":\"feature\"")
  let assert True = string.contains(s, "\"author_name\":\"Ada\"")
  let assert True = string.contains(s, "\"is_draft\":false")
}
...
pub fn merge_check_json_test() {
  ...
  let assert True = string.contains(s, "\"behind_target\":true")
  let assert True = string.contains(s, "\"conflict_paths\":[\"a.gleam\"]")
  let assert True = string.contains(s, "\"approval_count\":0")
  let assert True = string.contains(s, "\"required_approvals\":1")
}
...
pub fn commits_json_test() {
  let commits = [
    CommitEntry(sha: "abc", subject: "fix", author: "Ada", committed_at: "1"),
  ]
...
pub fn repo_detail_json_no_branch_test() {
  ...
  let s =
    json_api.repo_detail_json(repo, "git@h:acme/demo.git", option.None, option.None, 0)
    |> json_string
  let assert True = string.contains(s, "\"default_branch\":null")
  let assert True = string.contains(s, "\"required_approvals\":0")
}
...
pub fn merge_request_templates_json_test() {
  let templates = [
    RepoTemplate(
      name: "default",
      path: ".gleamhub/merge_request_template.md",
      content: "## Summary",
    ),
  ]
  let s =
    json_api.merge_request_templates_json("main", templates)
    |> json_string
  let assert True = string.contains(s, "\"name\":\"default\"")
  let assert True = string.contains(s, "## Summary")
}
...
pub fn merge_requests_list_json_includes_pipeline_test() {
  ...
  let assert True = string.contains(s, "\"id\":\"run-1\"")
  let assert True = string.contains(s, "\"trigger\":\"push\"")
}
...
pub fn merge_requests_json_test() {
  ...
  let s = json_api.merge_requests_json([mr], fn(_) { "Ada" }) |> json_string
...
pub fn merge_request_comment_json_test() {
  ...
  let s =
    json_api.merge_request_comment_json(comment, ["ada"])
    |> json_string
  let assert True = string.contains(s, "\"line\":1")
  let assert True = string.contains(s, "\"mentioned_user_ids\":[\"u2\"]")
}
...
pub fn merge_request_comments_json_test() {
  ...
  let s =
    json_api.merge_request_comments_json([comment], fn(_) { [] })
    |> json_string
  ...
  let s =
    json_api.me_json(
      ...
      UserStatsRow(
        ...
        open_issues_authored: 0,
        open_issues_assigned: 0,
        reviews_given: 0,
      ),
      0,
    )
...
pub fn access_json_write_test() {
  let s = json_api.access_json(False, True, "user_1") |> json_string
