...
pub fn ssh_authorized_keys_test() {
  db_test_support.with_db(fn(db) {
    ...
    let assert True =
      route_test_support.contains(hit, "GLEAMHUB_KEY_BLOB=" <> blob)
    ...
    let res =
      route_test_support.dispatch(
        route_test_support.get(
          "/internal/ssh/access?org=acme&repo=demo",
          option.None,
        ),
        ctx,
      )
...
pub fn ssh_access_check_test() {
  db_test_support.with_db(fn(db) {
    ...
    let #(public_key, blob, fp) = fixtures.test_ssh_key()
    let assert Ok(_) =
      database.insert_key(db, "owner", "laptop", public_key, blob, fp)

    let allowed =
      route_test_support.dispatch(
        route_test_support.internal_get(
          "/internal/ssh/access?org=acme&repo=demo&k="
          <> blob
          <> "&op=upload-pack",
        ),
        ctx,
    ...
    let denied =
      route_test_support.dispatch(
        route_test_support.internal_get(
          "/internal/ssh/access?org=acme&repo=demo&user_id=owner&op=upload-pack",
        ),
        ctx,
      )
...
pub fn ssh_access_resolves_key_for_org_member_test() {
  db_test_support.with_db(fn(db) {
    let root = route_test_support.repos_root()
    let #(ctx, _sign) = route_test_support.authenticated(db, root)
    let disk = "acme/demo.git"
    ...
    let assert Ok(_) =
      database.insert_key(db, "owner", "laptop", public_key, blob, fp)
    let assert Ok(_) =
      database.insert_key(db, "outsider", "other", public_key, blob, fp)
    ...
    let assert 200 = route_test_support.status(allowed)
    let assert True =
      route_test_support.contains(allowed, "\"user_id\":\"owner\"")
...
pub fn ssh_ref_update_unprotected_allows_test() {
  db_test_support.with_db(fn(db) {
    ...
    let res =
      route_test_support.dispatch(
        route_test_support.internal_get(
          "/internal/ssh/ref-update?org=acme&repo=demo&k="
          <> blob
          <> "&oldrev=0000000000000000000000000000000000000000&newrev=abc123def4567890123456789012345678901234&ref=refs/heads/feature",
        ),
        ctx,
      )
...
pub fn ssh_ref_update_rejects_user_id_without_key_test() {
  db_test_support.with_db(fn(db) {
    let root = route_test_support.repos_root()
    let #(ctx, _sign) = route_test_support.authenticated(db, root)
    let disk = "acme/demo.git"
