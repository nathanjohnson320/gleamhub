...
import http/clerk_api.{type Client}
import http/router
import http/web
import pog
import wisp
...
pub fn internal_post_form(path: String, body: String) -> wisp.Request {
  simulate.request(http.Post, path)
  |> simulate.header("x-gleamhub-internal-token", internal_api_token)
...
pub fn get(path: String, token: option.Option(String)) -> wisp.Request {
  ...
  case token {
    option.Some(t) -> simulate.header(req, "authorization", "Bearer " <> t)
    option.None -> req
  }
}

pub fn post(path: String, token: String) -> wisp.Request {
  simulate.request(http.Post, path)
  |> simulate.header("authorization", "Bearer " <> token)
}

pub fn post_json(path: String, token: String, body: json.Json) -> wisp.Request {
  simulate.request(http.Post, path)
  |> simulate.header("authorization", "Bearer " <> token)
  |> simulate.json_body(body)
}
...
pub fn response_header(response: wisp.Response, name: String) -> String {
  let wanted = string.lowercase(name)
  case
    list.find(response.headers, fn(pair) {
      let #(key, _) = pair
      string.lowercase(key) == wanted
    })
  {
    Ok(#(_, value)) -> value
    Error(_) -> ""
...
pub fn advance_branch(git_dir: String, branch: String) -> Nil
