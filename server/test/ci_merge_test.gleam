...
pub fn skipped_pipeline_allows_merge_test() {
  let git = git_exec.MergeCheck(mergeable: True, message: "")
  let check =
    ci_merge.combine_merge_check(git, option.Some(sample_run("skipped", "abc")), "abc")
...
pub fn success_at_head_allows_merge_test() {
  let git = git_exec.MergeCheck(mergeable: True, message: "")
  let check =
    ci_merge.combine_merge_check(git, option.Some(sample_run("success", "abc")), "abc")
  let assert True = check.mergeable
}
...
pub fn running_blocks_merge_test() {
  let git = git_exec.MergeCheck(mergeable: True, message: "")
  let check =
    ci_merge.combine_merge_check(git, option.Some(sample_run("running", "abc")), "abc")
...
pub fn failure_blocks_merge_test() {
  let git = git_exec.MergeCheck(mergeable: True, message: "")
  let check =
    ci_merge.combine_merge_check(git, option.Some(sample_run("failure", "abc")), "abc")
  let assert False = check.mergeable
}

pub fn git_conflict_overrides_pipeline_test() {
  let git = git_exec.MergeCheck(mergeable: False, message: "Merge conflicts")
  let check =
