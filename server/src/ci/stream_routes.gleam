  ...
  case wisp_req.method {
    http.Get ->
      case org_access.require_member(ctx, user_id(ctx), org_slug) {
        ...
        Ok(_) ->
          case
            database.get_merge_request(ctx.repo(), org_slug, repo_name, number)
          {
...
fn with_cors(
  ...
  cors_builder.set_cors_multiple_origin(
    response,
    cors(),
    request_origin(mist_req),
  )
}
